...
def get_option_inputs():
    ...
    expiration_date = st.sidebar.selectbox(
        "Select Expiration Date", expiration_dates)
 ...
 # Fetch the entire options chain for the selected date
    ...
    selected_contract = st.sidebar.selectbox(
        "Select Strike Price", active_contracts_df['strike'].tolist())

    selected_row = active_contracts_df[active_contracts_df['strike']
                                       == selected_contract].iloc[0]
    st.sidebar.write(
        f"Selected Contract: {selected_type} at Strike Price ${selected_contract:.2f}")

    # Get Inputs for Models
    T = (pd.to_datetime(expiration_date) - pd.to_datetime("today")
         ).days / 365.0  # Convert days to years
    st.sidebar.write(f"Time to Expiration (T): {T:.2f} years")
    r = st.sidebar.slider("Risk-Free Interest Rate (annualized)",
                          0.0, 1.0, 0.05, 0.01)  # Risk-free rate
...
def models():
    ...
    st.sidebar.write(
        f"Black-Scholes Price for {selected_type} Option: ${option_price_BSM:.2f}")
    # Calculate LSTM Price
    option_price_LSTM = pred_lstm(
        input_ticker, K, r, int(T/365), selected_type.lower())
    st.sidebar.write(
        f"LSTM Price for {selected_type} Option: ${option_price_LSTM:.2f}")
